...
s.close()
```
> chmod +x stream_sock.py
> ls

- In the other terminal execute these commands
```
nc -lp 1111                  # opens the port to recieve the message
echo "Kill" | nc -lp 1111    # sends the information back through the port
nc -lp 1111 < text.txt       # send the contents of the file to the port
```


## Datagram socket sender

> commands to run on your box for script
```
vim dgram_sock.py
control c control shift v
chmod +x dgram_sock.py
ls
```
> script
```
...
port = 2222
message = b"Vibe check..."
s.sendto(message, (ip_addr, port))
data, addr = s.recvfrom(1024)
print(data.decode())
```
> commands to run on the other box
```
ssh student@172.16.82.106
echo "Domain Expansion" | nc -luvp 2222
```



## Raw IPV4 and TCP Socket demo
...
message = b'Sigward'                  #This should be the student's last name per the prompt
...
```
> what you run for your box
```
sudo ./ip_raw.py
or
sudo !!
```
> what to run from their box
```
...
message = b'This is going to fast.'                                    # This should be the student's last name per the prompt
...
```
sudo tcpdump 'tcp[2:2] = 1776' -vvXXn
```
